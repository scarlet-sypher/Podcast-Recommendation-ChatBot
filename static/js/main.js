document.addEventListener("DOMContentLoaded", function () {
  ...
  function createPodcastDetailsModal() {
    ...
    if (!podcastDetailsModal) {
      ...
      document.addEventListener("keydown", function (event) {
        if (
          event.key === "Escape" &&
          podcastDetailsModal.classList.contains("show")
        ) {
          closePodcastDetailsModal();
        }
      });
  ...
  function showPodcastDetails(podcast) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (i < Math.floor(podcast.popularity / 2)) {
        ...
        i === Math.floor(podcast.popularity / 2) &&
        podcast.popularity % 2 >= 1
      ) {
    ...
    duration.innerHTML = `<i class="fas fa-clock"></i> <strong>Average episode:</strong> ${formatDuration(
      podcast.avg_episode_length
    )}`;
    ...
    const sampleEpisodes = [
      {
        title: `${podcast.title} - Latest Episode`,
        duration: Math.floor(Math.random() * 20) + podcast.avg_episode_length,
      },
      {
        title: `${podcast.title} - Fan Favorite`,
        duration:
          Math.floor(Math.random() * 20) + podcast.avg_episode_length - 5,
      },
      {
        title: `${podcast.title} - Special Guest`,
        duration:
          Math.floor(Math.random() * 20) + podcast.avg_episode_length + 10,
      },
    ];
    ...
    sampleEpisodes.forEach((episode) => {
      ...
      episodeDuration.innerHTML = `<i class="fas fa-clock"></i> ${formatDuration(
        episode.duration
      )}`;
      ...
      .then((data) => {
        ...
        setTimeout(() => {
          ...
          if (data.recommendations && data.recommendations.length > 0) {
            console.log(
              `Displaying ${data.recommendations.length} recommendations`
            );
  ...
  function displayPodcastsByCategory(category) {
    ...
    console.log(
      `Found ${filteredPodcasts.length} podcasts in category ${category}`
    );
    ...
    setTimeout(() => {
      ...
      if (filteredPodcasts.length === 0) {
        ...
        podcastGrid.innerHTML =
          '<p class="no-results">No podcasts found in this category.</p>';
        return;
      }
  ...
  function createPodcastCard(podcast, compact = false) {
    ...
    // Clone template
    const card = document
      .importNode(podcastCardTemplate.content, true)
    ...
    // Fill in data
    card.querySelector(".podcast-image img").src =
      podcast.image_url || "/static/images/podcasts/generic1.jpg";
    card.querySelector(".podcast-image img").alt = podcast.title;
    card.querySelector(".podcast-title").textContent = podcast.title;
    card.querySelector(
      ".podcast-host"
    ).textContent = `Hosted by ${podcast.host}`;
    ...
    card.querySelector(".podcast-duration span").textContent =
      formatDuration(avgDuration);

    // Description
    card.querySelector(".podcast-description").textContent =
      podcast.description;
  ...
  function performAdvancedSearch() {
    ...
    if (selectedDuration !== "any") {
      results = results.filter((podcast) => {
        const duration =
          podcast.avg_episode_length || podcast.avg_duration || 0;
    ...
    setTimeout(() => {
      ...
      if (results.length === 0) {
        ...
        searchResults.innerHTML =
          '<p class="no-results">No podcasts match your criteria. Try adjusting your filters.</p>';
        return;
      }
  ...
  function performPodcastSearch(query) {
    ...
    fetch("/api/search", {
      ...
      .catch((error) => {
        ...
        removeLoadingMessage();
        addMessageToChat(
          "Sorry, I couldn't complete your search. Please try again later.",
          "bot"
        );
  ...
  function displaySearchResults(query, results) {
    ...
    if (!results || results.length === 0) {
      // No results found
      addMessageToChat(
        `No podcasts found matching "${query}". Try a different search term.`,
        "bot"
      );
      return;
    }
    ...
    const summaryMessage = `Found ${results.length} podcast${
      results.length > 1 ? "s" : ""
    } matching "${query}":`;
